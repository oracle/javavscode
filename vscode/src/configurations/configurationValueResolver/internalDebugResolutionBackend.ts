...
import { completeInternalConfigurationResolver, createInternalResolverConfig, isInternalConfigurationResolverConfig } from "./utils";
...
export class InternalDebugResolutionBackend {
    ...
    public async resolveBatch(configs: ResolveConfigType[]): Promise<LSPAny[]> {
        if (configs.length === 0) {
            return [];
        }
        ...
        const resolution = new Promise<ResolverValueMap>(resolve => {
            const timeout = setTimeout(() => {
                ...
                this.completePendingInternalResolver(requestId);
    ...
    public completePendingInternalResolver(requestId: string, resolvedValues?: ResolverValueMap): void {
        ...
        pending.resolve(resolvedValues ?? pending.fallbackValues);
    }
    ...
    private createDebugConfig(requestId: string, resolverValues: ResolverValueMap): InternalResolverDebugConfiguration {
        return {
            ...createInternalResolverConfig(requestId, resolverValues),
