...
import { mergeConfigurations } from "./utils";
...
export class ConfigurationValueResolver implements IConfigurationValueResolver {
    ...
    private isPrefixAmongstSupportedKeys(prefixConfigKey: string): boolean {
        if (this.isSupported(prefixConfigKey)) {
            return true;
        }

        const sectionPrefix = `${prefixConfigKey}.`;
        for (const key of this.supportedConfigKeys) {
            if (key.startsWith(sectionPrefix)) return true;
        }
        return false;
    }
    ...
    public async resolveIfNeededBatch(configs: ResolveConfigType[]): Promise<LSPAny[]> {
        ...
        configs.forEach(({ configKey: prefixConfigKey, rawValue }, index) => {
            if (this.isPrefixAmongstSupportedKeys(prefixConfigKey)) {
                resolvableEntries.push({ index, configKey: prefixConfigKey, rawValue });
            }
        });
        ...
        try {
            ...
            resolvedBatch.forEach((value, idx) => {
                const { index, configKey, rawValue } = resolvableEntries[idx];
                resolvedValues[index] = mergeConfigurations(configKey, rawValue, value);
            });
        } catch (error: unknown) {
