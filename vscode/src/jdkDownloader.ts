...
export async function openJDKSelectionView(log: vscode.OutputChannel) {
  ...
  customView = vscode.window.createWebviewPanel(
    'jdkDownloader',
    l10n.translate("jdk.downloader.heading"),
  ...
  customView.webview.onDidReceiveMessage(async message => {
    ...
    if (command === 'downloadJDK') {
      ...
      if (installationPath) {
        ...
        vscode.window.showInformationMessage(l10n.translateMap("jdk.downloader.message.downloadingAndCompletingSetup",{
            jdkType:jdkType,
...
export function JDKDownloader(JDKType: string, osType: string, osArchitecture: string, JDKVersion: string, installationPath: string): void {
  ...
  const request = https.get(downloadUrl, response => {
    if (response.statusCode !== 200) {
      vscode.window.showErrorMessage(l10n.translateMap("jdk.downloader.error_message.downloadFailedHttpError", {
    ...
    writeStream.on('finish', async () => {
      ...
      if (checkSumExpected === checkSumObtained) {
        ...
        vscode.window.showErrorMessage(l10n.translateMap("jdk.downloader.message.downloadFailed",{
            "JDKType":JDKType,
            "JDKVersion":JDKVersion,
            "osType":osType
            }));
    ...
    writeStream.on('error', error => {
      vscode.window.showErrorMessage(l10n.translate("jdk.downloader.error_message.whileSavingFile") + error);
    });
  });

  request.on('error', error => {
        vscode.window.showErrorMessage(l10n.translateMap( "jdk.downloader.error_message.whileDownloading",{
        "JDKType":JDKType
        })+ error);
  });
...
export async function extractJDK(jdkTarballPath: string, extractionTarget: string, jdkVersion: string, osType: string, jdkType: string): Promise<void> {
  ...
  child_process.exec(extractCommand, async (error) => {
    if (error) {
      vscode.window.showErrorMessage(l10n.translate("jdk.downloader.error_message.jdkExtractionError") + error);
    } else {
      ...
      if (newDirectoryPath === null) {
        vscode.window.showInformationMessage(l10n.translateMap("jdk.downloader.error_message.jdkNewDirectoryIssueCannotInstall",{
            jdkType:jdkType,
            jdkVersion:jdkVersion,
...
const handleJdkPaths = async (directoryName: string, parentPath: string, osType: string): Promise<string | null> => {
  ...
  if (fs.existsSync(directoryPath)) {
    const CONFIRMATION_MESSAGE = l10n.translateMap("jdk.downloader.message.confirmation.directoryExistsStillWantToDelete",{
        name:name
        });
    const selected = await vscode.window.showInformationMessage(CONFIRMATION_MESSAGE, l10n.translate("jdk.downloader.message.confirmation.yes"), l10n.translate("jdk.downloader.message.confirmation.no"));
    if (selected === "Yes") {
...
const selectPath = async (installType: string): Promise<string | null> => {
  const options: vscode.OpenDialogOptions = {
    ...
    canSelectMany: false,
    openLabel: installType === 'manual' ? l10n.translate("jdk.downloader.label.selectJdk") : l10n.translate("jdk.downloader.label.installJdk")
  };
  ...
  if (selectedFolders && selectedFolders.length > 0) {
    ...
    vscode.window.showInformationMessage(l10n.translate("jdk.downloader.message.noLocationSelected"));
    return null;
  }
}
...
const installationCompletion = async (installType: string) => {
  ...
  if (installType === "automatic") {
    dialogBoxMessage = l10n.translate("jdk.downloader.message.completedInstallingJdk");
  } else {
    dialogBoxMessage = l10n.translate("jdk.downloader.message.addedJdkPath");
  }
  const selected = await vscode.window.showInformationMessage(dialogBoxMessage, l10n.translate("jdk.downloader.message.reload"));
...
export const fetchJDKDownloadView = (machineArch: string, osType: string, versions: Array<String>): string => {
 ...
 let downloader_title = l10n.translate("jdk.downloader.heading")
  return `<!DOCTYPE html>
  <head>
  ...
  <body>
    ...
    <button id="openJDK" class="select-jdk">${l10n.translate("jdk.downloader.button.label.openJdk")}</button> 
    ${l10n.translate("jdk.downloader.label.or")} 
    <button id="addJDKPathManually" class="select-jdk">${l10n.translate("jdk.downloader.button.label.selectJdkFromSystem")}</button>
    <br>
    <br>
    <div class="jdk-version-container" id="oracleJDKDiv">
      <div class="jdk-flex-basis">
        <label class="jdk-version-label">${l10n.translate("jdk.downloader.label.selectOracleJdkVersion")}</label>
        <br />
    ...
    return `<option value=${el}>JDK ${el}</option>`
      ...
      <div class="jdk-flex-basis">
        <label class="jdk-version-label">${l10n.translate("jdk.downloader.label.detectedOs")}</label>
        <br />
        <div class="jdk-version-dropdown">
          <select id="oracleJDKOsTypeDropdown">
            <option value="windows" ${osType === 'windows' ? 'selected' : null}>${l10n.translate("jdk.downloader.label.windows")}</option>
            <option value="macOS" ${osType === 'macOS' ? 'selected' : null}>${l10n.translate("jdk.downloader.label.mac")}</option>
            <option value="linux" ${osType === 'linux' ? 'selected' : null}>${l10n.translate("jdk.downloader.label.linux")}</option>
          </select>
        </div>
      </div>
      <div class="jdk-flex-basis">
        <label class="jdk-version-label">${l10n.translate("jdk.downloader.label.detectedMachineArchitecture")}</label>
        <br />
        <div class="jdk-version-dropdown">
      ...
      <div class="jdk-confirm-button">
        <button id="oracleJDKDownloadButton" class="select-jdk">${l10n.translate("jdk.downloader.button.label.downloadAndInstall")}</button>
      </div>
    </div>
    <div class="jdk-version-container" id="openJDKDiv">
      <div class="jdk-flex-basis">
        <label class="jdk-version-label">${l10n.translate("jdk.downloader.label.selectOpenJdkVersion")}</label>
        <br />
        <div class="jdk-version-dropdown">
    ...
    return `<option value=${el}>JDK ${el}</option>`
      ...
      <div class="jdk-confirm-button">
        <button id="openJDKDownloadButton" class="select-jdk">${l10n.translate("jdk.downloader.button.label.downloadAndInstall")}</button>
      </div>
    </div>
  </body>
  <script>
